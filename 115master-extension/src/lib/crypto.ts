...
export class Crypto115 {
  ...
  private kts = [
    ...
    47, 136, 216, 254, 254, 126, 134, 80, 149, 79, 209, 235, 131, 38, 52,
    219, 102, 123, 156, 126, 157, 122, 129, 50, 234, 182, 51, 222, 58,
    169, 89, 52, 102, 59, 170, 186, 129, 96, 72, 185, 213, 129, 156, 248,
    108, 132, 119, 255, 84, 120, 38, 95, 190, 232, 30, 54, 159, 52, 128,
    92, 69, 44, 155, 118, 213, 27, 143, 204, 195, 184, 245,
  ]
  private keyS = [0x29, 0x23, 0x21, 0x5E]
  private keyL = [120, 6, 173, 76, 51, 134, 93, 24, 76, 1, 63, 70]

  constructor() {
    this.rsa = new Rsa115()
  }

  private xor115Enc(src: number[], srclen: number, key: number[], keylen: number): number[] {
    const mod4 = srclen % 4
    const ret: number[] = []
  ...
  private getkey(length: number, key?: number[]): number[] {
    if (key) {
  ...
  private asymEncode(src: number[], srclen: number): string {
    const m = 128 - 11
    let ret = ''
  ...
  private asymDecode(src: number[], srclen: number): number[] {
    const m = 128
    let ret = ''
    for (let i = 0; i < Math.floor((srclen + m - 1) / m); i++) {
  ...
  private symEncode(src: number[], srclen: number, key1: number[], key2?: number[]): number[] {
  ...
  private symDecode(src: number[], srclen: number, key1: number[], key2: number[]): number[] {
    const k1 = this.getkey(4, key1)
    const k2 = this.getkey(12, key2)
  ...
  private bytesToString(buf: number[]): string {
...
export const crypto115 = new Crypto115()
